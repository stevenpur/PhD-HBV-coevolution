...
if (!("--covar_desc_size" %in% args)) {
    stop("Missing argument: --covar_desc_size")
}
...
covar_desc_size <- as.numeric(args[which(args == "--covar_desc_size") + 1])
...
covar_node <- sample(which(desc_n == covar_desc_size), 1)

# site4 and site5 have the co-mutation event on the node
descendants <- Descendants(tree, covar_node, type = "tips")[[1]]
