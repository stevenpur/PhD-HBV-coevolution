...
for (i in 1:nrow(comb_fu)){
    ...
    result_files <- list.files('./test/', pattern = paste0('simresult_', param_set, '_.*.txt'), full.names = TRUE)
    log_files <- list.files("./test/", pattern = paste0('log_', param_set, '_.*.rds'), full.names = TRUE)
    cur_data <- map(result_files, function(file){
        sim_ind <- gsub('.txt', '', strsplit(file, "_")[[1]][3])
        log_file <- paste0('./test/log_', param_set, '_', sim_ind, '.rds')
