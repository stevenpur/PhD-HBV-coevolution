#_test This script simulates sequences with coevolving sites given a tree.
...
if (!("--covar_desc_per" %in% args)) {
    stop("Missing argument: --covar_desc_per")
}
...
covar_desc_per <- as.numeric(args[which(args == "--covar_desc_per") + 1])
...
# set the run id
param_set <- paste0("l", seq_len, "n", pop_size, "f", coev_factor, "u", u, 'comut', covar_desc_per)
...
if (("--out" %in% args)) {
    outfile <- as.character(args[which(args == "--out") + 1])
} else {
    # set the output file
    outfile <- paste0("~/hbv_covar3/analysis/sim_seq/", param_set, "_test/simseq_", run_id, "rescaled.fasta")
}
...
# normalise the mutation rate
# pi <- rep(1/8, 8)
# u_current <- -sum(pi * diag(Qco3)) 
# scale_factor <- u / u_current
# Qco3 <- Qco3 * scale_factor
#
# for (i in 1:nrow(Qco3)) {
#     Qco3[i, i] <- -sum(Qco3[i, -i])
# }
#
...
write.dna(sim_msa_comut, outfile, format = "fasta", colsep = "")
